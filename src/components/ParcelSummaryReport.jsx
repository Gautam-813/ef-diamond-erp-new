...
const ParcelSummaryReport = ({ parcel, tender, state, prices, totals, onUpdate }) => {
  ...
  return (
    <div className="summary-report-container">
      ...
      {/* NEW SECTION: DETAILED POLISH CATEGORY SPLIT (GRANULAR) */}
      <div className="section" style={{ borderTop: '2px solid #eee', paddingTop: 20 }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 30 }}>
          {[
            { shape: 'Round', group: 'high', label: 'ROUND VVS-VS2', clarities: ['VVS', 'VS1', 'VS2'] },
            { shape: 'Round', group: 'low', label: 'ROUND SI1-I2', clarities: ['SI1', 'SI2', 'I1', 'I2'] },
            { shape: 'Fancy', group: 'high', label: 'FANCY VVS-VS2', clarities: ['VVS', 'VS1', 'VS2'] },
            { shape: 'Fancy', group: 'low', label: 'FANCY SI1-I2', clarities: ['SI1', 'SI2', 'I1', 'I2'] }
          ].map(tableCfg => {
            const scp = usableData.shapeClarityCategoryProfile?.[tableCfg.shape] || {};
            const availableRanges = ranges.filter(r => Object.keys(scp).includes(r));
            const avgSize = avgs?.[tableCfg.shape]?.[tableCfg.group] || 0;

            if (availableRanges.length === 0) return null;

            return (
              <div key={tableCfg.label}>
                <div className="section-title" style={{ color: '#1e3a8a', display: 'flex', justifyContent: 'space-between' }}>
                  <span>{tableCfg.label}</span>
                  <span style={{ color: '#b45309' }}>Avg Size: {formatNum(avgSize, 3)}</span>
                </div>
                <table className="summary-table mini">
                  <thead>
                    <tr>
                      <th>Clarity</th>
                      {availableRanges.map(r => <th key={r}>{r}</th>)}
                      <th>Total</th>
                    </tr>
                  </thead>
                  <tbody>
                    {tableCfg.clarities.map(clr => {
                      const rowTotalCts = availableRanges.reduce((sum, r) => sum + (scp[r]?.[clr]?.cts || 0), 0);
                      const rowTotalPcs = availableRanges.reduce((sum, r) => sum + (scp[r]?.[clr]?.pcs || 0), 0);
                      if (rowTotalCts === 0) return null;
                      ...
                      return (
                        <tr key={clr}>
                          ...
                          <td style={{ fontWeight: 700, backgroundColor: '#f9fafb' }}>
                            {formatNum(rowTotalCts, 2)} / {formatNum(rowTotalPcs, 0)}
                          </td>
                        </tr>
                      );
                    ...
                    <tr className="total-row">
                      <td>TOTAL</td>
                      {availableRanges.map(r => {
                        const colTotalCts = tableCfg.clarities.reduce((sum, clr) => sum + (scp[r]?.[clr]?.cts || 0), 0);
                        const colTotalPcs = tableCfg.clarities.reduce((sum, clr) => sum + (scp[r]?.[clr]?.pcs || 0), 0);
                        return <td key={r}>{formatNum(colTotalCts, 2)} / {formatNum(colTotalPcs, 0)}</td>;
                      })}
                      <td>
                        {formatNum(availableRanges.reduce((sum, r) => sum + tableCfg.clarities.reduce((s, clr) => s + (scp[r]?.[clr]?.cts || 0), 0), 0), 2)} / 
                        {formatNum(availableRanges.reduce((sum, r) => sum + tableCfg.clarities.reduce((s, clr) => s + (scp[r]?.[clr]?.pcs || 0), 0), 0), 0)}
                      </td>
                    </tr>
                  </tbody>
                </table>
